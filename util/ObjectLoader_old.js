...
Face.prototype.getNormal = function(vertices){
  var startIndex = this.indices[0];
  var v0 = vec3.fromValues(
    vertices[startIndex+3] - vertices[startIndex], 
    vertices[startIndex+4] - vertices[startIndex+1], 
    vertices[startIndex+5] - vertices[startIndex+2]);
  var v1 = vec3.fromValues(
    vertices[startIndex+6] - vertices[startIndex], 
    vertices[startIndex+7] - vertices[startIndex+1], 
    vertices[startIndex+8] - vertices[startIndex+2]);
  var normal = vec3.create();
  return vec3.cross(normal, v0, v1)
}
...
p.generateNormals = function(){
  var vertexMultiNormals = new Array();

  for(var i=0; i<this._faces.length; ++i){
    var face = this._faces[i];
    var faceNormal = face.getNormal(this._vertices);
    for(var j=0; j<face.indices.length; ++j){
      if(!vertexMultiNormals[face.indices[j]])
        vertexMultiNormals[face.indices[j]] = new Array();
      vertexMultiNormals[face.indices[j]].push(faceNormal);
    }
    // console.log(face.indices);
  }

  for(var i=0; i<vertexMultiNormals.length; ++i){
    var normals = vertexMultiNormals[i];
    var vertexNormal = normals[0];
    for(var j=1; j<normals.length; ++j){
      vec3.add(vertexNormal, vertexNormal, normals[j])
    }
    vec3.normalize(vertexNormal, vertexNormal);

    for(var k=0; k<vertexNormal.length; ++k){
      this._normals.push(vertexNormal[k]);
    }
  }

  // console.log(this._normals.length, this._vertices.length);
}
